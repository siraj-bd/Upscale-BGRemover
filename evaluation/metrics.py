...
def load_images_parallel(gt_paths, pred_paths, num_workers, verbose):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        if verbose:
            results = list(
                tqdm(
                    executor.map(load_single_image_pair, args_list),
                    total=len(args_list),
                    desc="Loading images",
                )
            )
    ...
    if num_workers:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            if verbose:
                results = list(
                    tqdm(
                        executor.map(process_with_measures, args_list),
                        total=len(args_list),
                ...
                for metric_name in metrics:
                    if metric_name in measures and metric_name in temp_measures:
                        ...
                        if hasattr(measures[metric_name], "sms"):
                            measures[metric_name].sms.extend(
                                temp_measures[metric_name].sms
                            )
                        if hasattr(measures[metric_name], "maes"):
                            measures[metric_name].maes.extend(
                                temp_measures[metric_name].maes
                            )
                        if hasattr(measures[metric_name], "mses"):
                            measures[metric_name].mses.extend(
                                temp_measures[metric_name].mses
                            )
                        if hasattr(measures[metric_name], "adaptive_fms"):
                            measures[metric_name].adaptive_fms.extend(
                                temp_measures[metric_name].adaptive_fms
                            )
                            measures[metric_name].precisions.extend(
                                temp_measures[metric_name].precisions
                            )
                            measures[metric_name].recalls.extend(
                                temp_measures[metric_name].recalls
                            )
                            measures[metric_name].changeable_fms.extend(
                                temp_measures[metric_name].changeable_fms
                            )
                        if hasattr(measures[metric_name], "adaptive_ems"):
                            measures[metric_name].adaptive_ems.extend(
                                temp_measures[metric_name].adaptive_ems
                            )
                            measures[metric_name].changeable_ems.extend(
                                temp_measures[metric_name].changeable_ems
                            )
                        if hasattr(measures[metric_name], "weighted_fms"):
                            measures[metric_name].weighted_fms.extend(
                                temp_measures[metric_name].weighted_fms
                            )
                        if hasattr(measures[metric_name], "hces"):
                            measures[metric_name].hces.extend(
                                temp_measures[metric_name].hces
                            )
                        if hasattr(measures[metric_name], "bas"):
                            measures[metric_name].bas.extend(
                                temp_measures[metric_name].bas
                            )
                            # MBA 需要合并累积统计
                            measures[metric_name].all_h += temp_measures[
                                metric_name
                            ].all_h
                            measures[metric_name].all_w += temp_measures[
                                metric_name
                            ].all_w
                            measures[metric_name].all_max += temp_measures[
                                metric_name
                            ].all_max
                        if hasattr(measures[metric_name], "bious"):
                            measures[metric_name].bious.extend(
                                temp_measures[metric_name].bious
                            )
    else:
        # 串行处理
...
def collect_results(measures, metrics):
    em = (
        measures["E"].get_results()["em"]
        if "E" in metrics
        else {"curve": np.array([np.float64(-1)]), "adp": np.float64(-1)}
    )
    sm = measures["S"].get_results()["sm"] if "S" in metrics else np.float64(-1)
    fm = (
        measures["F"].get_results()["fm"]
        if "F" in metrics
        else {"curve": np.array([np.float64(-1)]), "adp": np.float64(-1)}
    )
    ...
    biou = (
        measures["BIoU"].get_results()["biou"]
        if "BIoU" in metrics
        else {"curve": np.array([np.float64(-1)])}
    )
...
def evaluator(
    ...
    if use_parallel:
        if verbose:
            print(
                f"Loading {len(gt_paths)} images in parallel with {num_workers} workers..."
            )
        ...
        measures = process_metrics_batch(
            image_data, measures, metrics, verbose, num_workers
        )
    else:
        iterator = (
            tqdm(
                range(len(gt_paths)),
                total=len(gt_paths),
                desc="Loading images and computing metrics",
            )
            if verbose
    ...
    if verbose:
        ...
        print(
            f"Evaluation completed in {total_time:.2f}s using {processing_method} processing"
        )
...
def sort_and_round_scores(task, scores, r=3):
    ...
    if task == "DIS5K":
        scores = [
            fm["curve"].max().round(r),
            wfm.round(r),
            mae.round(r),
            sm.round(r),
            em["curve"].mean().round(r),
            int(hce.round()),
            em["curve"].max().round(r),
            fm["curve"].mean().round(r),
            em["adp"].round(r),
            fm["adp"].round(r),
            mba.round(r),
            biou["curve"].max().round(r),
            biou["curve"].mean().round(r),
        ]
    elif task == "COD":
        scores = [
            sm.round(r),
