...
def do_eval(args):
    ...
    for data_name in args.data_lst.split("+"):
        ...
        tb.vertical_char = "&"
        tb.field_names = (
            task_to_field_names[config.task]
            if config.task in task_to_field_names
            else task_to_field_names["Others"]
        )
        for model_name in args.model_lst[:]:
            print("\t", "Evaluating model: {}...".format(model_name))
            pred_paths = [
                p.replace(
                    args.gt_root, os.path.join(args.pred_root, model_name)
                ).replace("/gt/", "/")
                for p in gt_paths
            ]
            ...
            scores = sort_and_round_scores(
                config.task, [em, sm, fm, mae, mse, wfm, hce, mba, biou]
            )
            ...
            with open(
                os.path.join(args.save_dir, "{}_eval.txt".format(data_name)), "w+"
            ) as file_to_write:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_dir", type=str, help="directory to save results", default="e_results"
    )
    parser.add_argument(
        "--metrics",
        type=str,
